...
#knit: pagedown::chrome_print(outline = TRUE)
---
...
publications <- get_publications(id) %>%
  ...
  transmute(title = str_to_sentence(title),
         ...
         title = str_replace(title, "Australian", "australian"),
...
- steamlined Github and rMarkdown workflow for generating and updating manuscripts, reports, presentations, APIs, interactive web pages and applications, and also academic resumes!
...
Career Summary {data-icon=map-signs data-concise=true}
--------------------------------------------------------------------------------

Anthony is a sociologist with advanced geospatial statistics training that is excited by the promise of smart cities and a socially sustainable global society starting from the neighborhood-scale. Their overarching career objective is to clear the open and big data expertise barrier so that urban researchers and the public can co-design livable cities where socially diverse communities can thrive.
...
Key Publications {data-icon=file}
--------------------------------------------------------------------------------

### Total cites = `r total_cites`, h-index = `r round(h_index,0)`, and projected 10-year h-index projection = `r filter(predicted_h_index, years_ahead == 10) %>% pull(h_index) %>% round(., 1)` <sup>1</sup>

<sup>1</sup> Future impact: Predicting scientific success (Acuna et al. 2012 in *Nature*) 
...
Key Conference Presentations {data-icon=microphone}
--------------------------------------------------------------------------------
...
**Aim**: competitive grant for travel and registration to present the 2019 Australian Sociological Association conference
...
Appendix {data-icon=folder data-concise=true}
--------------------------------------------------------------------------------
...
Full Publications   {data-icon=file}
--------------------------------------------------------------------------------
...
Full Conference Presentations {data-icon=microphone}
--------------------------------------------------------------------------------
